<template>
  <div
    ...
    v-loading="loading"
  >
    <p class="title">{{movie.title}}<span v-if="movie.title!==movie.original_title">{{movie.original_title}}</span><span class="year">({{movie.year}})</span>
    </p>
    <div class="info">
      ...
      <div class="movie-info">
        <p>导演：<span
          ...
          >{{item.name}}/</span></p>
        <p>编剧：<span
            v-for="(item, index) in movie.writers"
            :key="'writer'+index"
          >{{item.name}}/</span></p>
        <p>主演：<span
            v-for="(item, index) in movie.casts"
            :key="'cast'+index"
          >{{item.name}}/</span></p>
        <p>类型：<span
        ...
        <p>制片国家/地区：<span
          ...
          >{{item}}/</span></p>
        <p>语言：<span
            v-for="(item, index) in movie.languages"
            :key="'lan'+index"
          >{{item}}/</span></p>
        <p>上映日期：<span
            v-for="(item, index) in movie.pubdates"
            :key="'pub'+index"
          >{{item}}/</span></p>
        <p>片长：<span
      ...
      <div class="movie-rate">
        ...
        ???<el-progress color="#FFD596" :text-inside="true" :stroke-width="14" :percentage="70"></el-progress>
        ???<el-progress color="#FFD596" :text-inside="true" :stroke-width="14" :percentage="70"></el-progress>
        ???<el-progress color="#FFD596" :text-inside="true" :stroke-width="14" :percentage="70"></el-progress>
    ...
    <div class="celebrities">
      ...
      <div class="cele-contain">
        <div class="cele-item" v-for="(director, index) in movie.directors" :key="'director'+index">
          <div class="img-contain">
            <img :src="director.avatars.medium" alt="">
          </div>
          <p>{{director.name}}</p>
        </div>
        <div class="cele-item"  v-for="(cast, index) in movie.casts" :key="index">
          <div class="img-contain">
    ...
    <div class="comments-section">
      ...
      <div class="comment-contain" v-for="(item, index) in movie.popular_comments" :key="index">
        <div class="top">
